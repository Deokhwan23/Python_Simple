...
#   - 여러분들이 아이디어가 떠롤랐을 때 바로 구현은 못하지만 구글링해서 어떻게 어떻게하면 만들 수 있겠네라는 생각이 들 정도
...
print(id.lower().strip()) # "cholong1004" # . = 참조연산자 ~에서

# 4-6. find() and rfind() : 문자열 내부의 특정 문자 위치 인덱스 출력
# find()는 좌측에서 우측으로 찾고, rfind()는 우측에서 좌측으로 찾음
...
print(str.rfind("World"))   # 단어의 첫글자 인덱스
